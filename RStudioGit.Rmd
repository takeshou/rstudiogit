...
RStudioでのみGitを使っていると、2行目以降のコメントは読めないが、GitHubなどリモートにPushすると見ることが出来る。
...
いくつかの作業は、Gitタブ->More->Shellを使うか、Git Bashを使ってGitの作法に従って行うか、バッドノウハウ的に行う必要がある。
...
共有・共同開発・バックアップなどいろんな用途があるが、リモートで管理する場合は、先にGitHub\footnote{https://github.com/}やBitbucket\footnote{https://bitbucket.org/}の登録が必要となる。
登録にはメールアドレスが必要だが、大学のアドレスを使うと、Bitbucketはアカデミックライセンスとなって便利だし、GitHubは大学のアドレスを登録した後、申請すればアカデミックライセンスになってプライベートリポジトリ（非公開）が使えるようになる。
申請が面倒な場合は、Bitbucketだと最初からプライベートリポジトリが使えるので、そちらを登録すると良い。
ただし、世の中的には、GitHubが標準的なものとして扱われているので、注意すること。
